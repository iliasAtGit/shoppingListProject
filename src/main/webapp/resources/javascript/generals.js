...
/*
 * function toggleElements(id, tdClass) id: a unique id, mostly the sql record
 * id tdClass: array of td classes that contain the divs to toggle
 * 
 * Affected style <tr id="tr${id}"> <td class="tdClass"> <div class="read"></div>
 * <!-- .read initialized as visible --> <div class="write"></div> <!-- .write
 * initialized as display: none; --> </td> </tr>
 * 
 */
function toggleElements(id, tdClass) {
	if (tdClass.length != 0) {
		newReadElementVisibility = ($(
				"#tr" + id + " ." + tdClass[0] + " div:nth-child(1)").is(
				':visible') === true) ? false : true;

		for (i = 0; i < tdClass.length; i++) {
			$("#tr" + id + " ." + tdClass[i] + " div:nth-child(1)").toggle(
					newReadElementVisibility);
			$("#tr" + id + " ." + tdClass[i] + " div:nth-child(2)").toggle(
					!newReadElementVisibility);
		}
	}
}
...
function showErrorCodesAfterRequest(id, responseJSON) {
	...
	$.each(responseJSON.fieldErrors, function(key, item) {
		$("#tr" + id + " ." + item.field + " div.has-error").append(
				item.code + "<br />")
	});
}
